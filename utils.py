...
def create_adj_from_dict(triples, num_ents, num_rels, threshold):
    ...
    """
    r1r2_set = Counter(["{}_{}".format(ele[0], ele[2]) for ele in triples]) # number of times h and r appear together 计数h r
    # print(r1r2_set)
    ...
    for triple in triples:
        ...
        for v_index in values:
            if values[v_index] != 0:
                ...
                dat_values.append(values[v_index])
    edge_index = torch.LongTensor([row_indxs, col_indxs])
        ...
        for ele in as_tail_set:
            for ele2 in as_head_set:
                ...
                if str_key not in new_triple_text:
                    ...
                    new_triple.append([ele, ent, ele2])#<rel, ent, rel>

    print("Number of ori triples: {}".format(len(data)))
    print("Number of rel triples: {}".format(len(new_triple)))
    edge_mat,edge_index = create_adj_from_dict(new_triple, len(entity_idxs), len(rel_idxs), threshold)
    return edge_mat,edge_index
...
#有向的邻接矩阵
def dir_construct_entity_focus_matrix(data, entity_idxs):
    row_indxs = []
    col_indxs = []
    dat_values = []
    for hrt in data:
...
def create_dir_adj_from_dict(triples, num_ents, num_rels, threshold):
    """
    Parameter:
    triples: list
        - triples (each triple is a list of 3 elements)
    ...
    """
    r1r2_set = Counter(
        ["{}_{}".format(ele[0], ele[2]) for ele in triples])  # number of times h and r appear together 计数h r
    # print(r1r2_set)
    co_times = np.sort(list(r1r2_set.values()))
    ...
    for triple in triples:
        ...
        for v_index in values:
            if values[v_index] != 0:
                ...
                # dat_values.append(values[v_index])
                # dat_values.append(values[v_index])
                row_indxs.append(list_1[v_index])  # 添加行索引
                col_indxs.append(list_2[v_index])  # 添加列索引
                dat_values.append(values[v_index])  # 添加数据值
    edge_index = torch.LongTensor([row_indxs, col_indxs])
...
def construct_dir_relation_focus_matrix(data, entity_idxs, rel_idxs, threshold):
    """
    construct the relation-focus graph
    """
    num_relations = int(len(rel_idxs) / 2)  # does not count the inversed relations
    ...
    print("Number of ori triples: {}".format(len(data)))
    print("Number of rel triples: {}".format(len(new_triple)))
    edge_mat,edge_index = create_dir_adj_from_dict(new_triple, len(entity_idxs), len(rel_idxs), threshold)
    return edge_mat,edge_index
...
def get_ert_vocab(data):
    ert_vocab = defaultdict(list) # er_vocab is a dict of (h, r) and t_index, if r is a reverse relation ==> t_index += n_entities
    for triple in data:
        ert_vocab[(triple[0], triple[1],triple[2])].append(triple[2])
    return ert_vocab


def get_t_vocab(data):
    t_vocab = defaultdict(list) # er_vocab is a dict of (h, r) and t_index, if r is a reverse relation ==> t_index += n_entities
    for triple in data:
        t_vocab[(triple[2])].append(triple[2])
    return t_vocab
...
from torch_geometric.utils import subgraph, k_hop_subgraph, sort_edge_index, dropout_adj, add_self_loops, degree, \
...
class Compose():
    ...
    def augment(self, x, edge_index, edge_type, rel_emb, edge_batch_idx=None):
        for aug in self.augmentors:
            x, edge_index, edge_type, rel_emb = aug.augment(x=x, edge_index=edge_index, edge_type=edge_type,
                                                            rel_emb=rel_emb, edge_batch_idx=edge_batch_idx)
...
class Random_Add_Edge():
    ...
    def augment(self, x, edge_index, edge_type, rel_emb=None, edge_batch_idx=None):
        ...
        edge_index, edge_type = edge_index_coalesce(edge_index=edge_index, edge_attr=edge_type,
                                                    num_nodes=num_nodes + num_add, is_sorted=True, reduce="mean")
...
class Local_Restruct():
    ...
    def augment(self, x, edge_index, edge_type, rel_emb=None, edge_batch_idx=None):
        ...
        edge_index, edge_type = sort_edge_index(edge_index, edge_type)
        edge_index, edge_type = edge_index_coalesce(edge_index=edge_index, edge_attr=edge_type,
                                                    num_nodes=self.num_nodes, is_sorted=True, reduce="max")
...
class Add_2hop_Neighbors():
    ...
    def augment(self, x, edge_index, edge_type, rel_emb=None, edge_batch_idx=None):
        ...
        edge_index, edge_type = sort_edge_index(edge_index, edge_type)
        edge_index, edge_type = edge_index_coalesce(edge_index=edge_index, edge_attr=edge_type, num_nodes=self.num_node,
                                                    is_sorted=True, reduce="max")
        # torch.cuda.empty_cache()
        return x, edge_index, edge_type.long(), rel_emb
...
class Path_Ablation():
    ...
    def augment(self, x, edge_index, edge_type, rel_emb=None, edge_batch_idx=None):
        ...
        for _ in range(self.num_hop - 1):
            ...
            select_edge_index, value = spspmm(select_edge_index, value, select_edge_index, value, N, N, N,
                                              coalesced=True)
            value.fill_(0)
